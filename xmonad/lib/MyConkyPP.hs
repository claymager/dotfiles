...
module MyConkyPP (conkyPP) where
...
processWorkspace offset color symbol name =
...
myLogOrder (ws:rest) = (splitToRows ws) ++ rest
  where
    splitToRows ws = (workspaceRowPrefix++) <$> (intercalate " ") <$> ($lines ws) <$> splitFunctions
    splitFunctions = [
      take 3,               -- [ [ 1 2 3 ]
      take 3 . drop 3,      --   [ 4 5 6 ]
      drop 6 ]              --   [ 7 8 9 ] ]
...
layoutMap = unlines . map (layoutSymbolPrefix ++) . highlightSymbol . last . words
  where
...
applyFont font size text = "${font " ++ font ++ ":pixelsize=" ++ (show size) ++ "}" ++ text

unicode :: String -> String
unicode code = "${exec printf \\\\u" ++ code ++ "}"

conkyVOffset :: Int -> String -> String
conkyVOffset n text = "${voffset "++ show n ++ "}" ++ text ++ "${voffset "++ show (-n) ++ "}"
